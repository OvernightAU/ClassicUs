...
public static class AssetBundleManager
{
    ...
    private static bool TryFindFile(Assembly assembly, string fileName, [NotNullWhen(true)] out string? path)
    {
        ...
        if (pluginDirectoryPath != null)
        {
            ...
            {
                Debug.Log($"Loading an asset bundle from {filePath}");
                path = filePath;
                return true;
            }
        }
    ...
    {
        var resourceName = assembly.GetManifestResourceNames().SingleOrDefault(n => n.EndsWith(fileName, System.StringComparison.Ordinal));
        if (resourceName != null)
        {
            Debug.Log($"Loading an asset bundle from {resourceName}");

            using var stream = assembly.GetManifestResourceStream(resourceName) ?? throw new System.InvalidOperationException("Resource stream was null");
